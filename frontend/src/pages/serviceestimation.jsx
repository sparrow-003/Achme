...
const ServiceEstimation = () => {
  ...
  const formatSubSENumber = (rootId, version, dateStr) => {
    const year = dateStr ? new Date(dateStr).getFullYear() : new Date().getFullYear();
  ...
  const handleEdit = async (id) => {
    ...
    setServiceInvoice({ invoice_date: h.invoice_date?.split("T")[0] || "" });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!serviceInvoice.invoice_date) return alert("Please select date");
    ...
    } catch (err) { console.error(err); alert("Error saving Service Estimation"); }
  };
  ...
  const resetForm = () => {
    ...
    setDescInput("");
    setServiceInvoice({ invoice_date: (() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; })() });
    setExtra(emptyExtra());
    setEditId(null);
    setEditingIndex(null);
  };
  ...
  const filteredInvoices = serviceInvoices.filter(q => q.customer_name?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="w-full">
      ...
      <div className="invoice-heading-tab flex gap-4 justify-between items-center flex-wrap">
        <div>
          <h2 className="text-2xl font-bold text-[#1694CE]">Service Estimation</h2>
          <nav className="text-sm text-gray-500">Dashboard &gt; Finance &gt; Service Estimation</nav>
        </div>
        ...
        <table className="w-full text-sm text-center border-collapse min-w-[600px]">
          <thead className="bg-[#f8fafc]">
            <tr className="text-gray-700 font-bold uppercase text-xs border-b border-gray-200">
              <th className="px-4 py-4 border-r">SE Number</th>
          ...
          <tbody>
            {filteredInvoices.map(p => (
              <tr key={p.id} onClick={() => setSelectedId(p.id)} onDoubleClick={() => { setViewId(p.id); setTimeout(() => setShowInvoice(true), 50); }}
                ...
                <td className="px-4 py-4 border-r font-medium text-blue-600">{formatSENumber(p.id, p.invoice_date)}</td>
      ...
      <div className={`overlay ${open ? "show" : ""} flex justify-center items-start overflow-y-auto pt-6 pb-10`}>
        <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-5xl p-8 relative">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-gray-800">{editId ? "Edit Service Estimation" : "Create Service Estimation"}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input type="date" value={serviceInvoice.invoice_date} onChange={e => setServiceInvoice({ ...serviceInvoice, invoice_date: e.target.value })} className="border rounded-lg px-3 py-2 outline-none text-sm" required />
              </div>
            </div>
              ...
              <div className="space-y-2">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Standard terms apply to this service estimation</p>
                  </div>
                </label>
            ...
            <div className="flex gap-4 pt-4">
              <button type="submit" className="bg-blue-600 text-white px-10 py-2.5 rounded-lg hover:bg-blue-700 font-bold shadow-lg transition">Save Service Estimation</button>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-3xl p-6">
            ...
            {(() => {
              const filtered = historyList.filter(q => {
                const subNum = formatSubSENumber(q.parent_id || historyRootId, q.version, q.invoice_date).toLowerCase();
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead className="bg-gray-50">
                      <tr className="text-gray-600 font-bold uppercase text-xs border-b">
                        <th className="px-4 py-3 text-left">Sub-SE Number</th>
                      ...
                      {filtered.map(q => (
                        <tr key={q.id} onClick={() => setHistorySelectedId(q.id)}
                          ...
                          <td className="px-4 py-3 font-semibold text-blue-600">
                            {formatSubSENumber(q.parent_id || historyRootId, q.version, q.invoice_date)}
      ...
      <div className={`overlay ${mailOpen ? "show" : ""} flex justify-center items-center`}>
        <div className="bg-white rounded-xl shadow-2xl w-[90%] max-w-lg p-8 relative">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"><Mail size={20} /> Send Service Estimation</h2>
